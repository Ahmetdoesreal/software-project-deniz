...
try:
    ...
    def current_process_usernames(extra_usernames=None) -> set[str]:
        usernames = {"admin", "administrator", "root", "system", "trustedinstaller"}
        if extra_usernames:
            usernames.update(
                normalized
                for username in extra_usernames
                if (normalized := normalize_process_username(username))
            )
        return usernames
...
class ClientIncidentEngine:
    ...
    def _observe_rapid_application_switching(self, snapshot: dict) -> list[dict]:
        ...
        timestamp = str(snapshot.get("timestamp") or snapshot.get("event_at") or _now_iso())
