...
def process_blacklist(entries: list[str], version: str, process_usernames: list[str] | None = None) -> str:
    ...
    return protocol.encode(
        PROCESS_BLACKLIST,
        {
            "entries": entries,
            "version": version,
            "process_usernames": process_usernames or [],
        },
    )
...
CLIENT_MONITOR_EVENT = "client_monitor_event"

def client_monitor_event(payload: dict) -> str:
    """Client sends periodic structured monitor telemetry to the server."""
    return protocol.encode(CLIENT_MONITOR_EVENT, payload)
