...
from common.process_definitions import normalize_actions
...
def incident_matches_rule(incident: dict, raw_rule: dict) -> bool:
    ...
    if process_names:
        ...
        allowed = {_process_name(value) for value in process_names if _process_name(value)}
        if process_name not in allowed:
            return False
    ...
    if browser_names:
        configured = True
        process_name = _process_name(incident.get("process_name"))
        allowed = {_process_name(value) for value in browser_names if _process_name(value)}
        if process_name not in allowed:
            return False
