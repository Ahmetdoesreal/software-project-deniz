...
PROCESS_FIELDS = ("pid", "name", "username", "exe")


def get_processes_via_psutil() -> set[tuple[int, str, str | None, str | None]]:
    processes = set()
    try:
        iterator = psutil.process_iter(PROCESS_FIELDS, ad_value=None)
        for process in iterator:
            try:
                ...
                processes.add((
                    ...
                    str(process_path) if process_path else None,
                ))
...
def _print_processes(processes: set[tuple[int, str, str | None, str | None]], limit: int) -> None:
    for pid, name, username, process_path in sorted(processes, key=lambda item: (str(item[1]).lower(), int(item[0])))[:limit]:
        print(f"{pid}\t{name}\t{username or ''}\t{process_path or ''}")
