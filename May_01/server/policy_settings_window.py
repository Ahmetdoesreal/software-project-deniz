...
SETTINGS_TAB_LABELS = {
    ...
    "Process Path Clarification": "Path Check",
...
class PolicySettingsMixin:
    ...
    def _build_policy_settings_window(self, parent):
        ...
        self._build_path_clarification_settings_section()
    ...
    def _build_focused_window_settings_section(self):
        ...
        self._settings_combo(frame, "Title Match", "rules.focused_window.window_title_match_mode", 1, 0, WINDOW_TITLE_MATCH_MODES)
    ...
    def _build_rapid_switch_settings_section(self):
        ...
        self._settings_entry(frame, "Window Seconds", "rules.rapid_application_switching.window_seconds", 1, 2, width=12)
        self._settings_entry(frame, "Window Observations", "rules.rapid_application_switching.window_observations", 2, 0, width=12)
        self._settings_check(frame, "rules.rapid_application_switching.auto_violation_pause", "Auto pause on violation", 2, 2, columnspan=2)
    ...
    def _build_unexpected_process_settings_section(self):
        ...
        self._settings_check(frame, "rules.unexpected_process.baseline_existing_processes", "Baseline existing processes", 1, 0, columnspan=2)
        self._settings_check(frame, "rules.unexpected_process.auto_violation_pause", "Auto pause on violation", 1, 2, columnspan=2)
        self._settings_text(frame, "Known process names, one per line", "rules.unexpected_process.known_process_names", 2, height=4)
        self._settings_text(frame, "Known directory paths, one per line", "rules.unexpected_process.known_directory_paths", 3, height=4)
        self._settings_text(frame, "Allowed process names, one per line", "rules.unexpected_process.allowed_process_names", 4, height=4)

    def _build_process_definition_settings_section(self):
        frame = self._settings_section("Process Definitions")
        self._settings_check(frame, "rules.process_definitions.enabled", "Enabled", 0, 0)
        self._settings_combo(frame, "Severity", "rules.process_definitions.severity", 0, 1, SEVERITY_VALUES)
        self._settings_check(frame, "rules.process_definitions.detect_unknown_processes", "Detect unknown processes", 1, 0, columnspan=2)
        self._settings_combo(frame, "Unknown Severity", "rules.process_definitions.unknown_severity", 1, 2, SEVERITY_VALUES)
        self._settings_check(frame, "rules.process_definitions.baseline_existing_processes", "Baseline existing processes", 2, 0, columnspan=2)
        self._settings_check(frame, "rules.process_definitions.auto_violation_pause", "Auto pause on violation", 2, 2, columnspan=2)
        self._settings_check(frame, "rules.process_definitions.allow_remote_kill", "Allow remote kill", 3, 0, columnspan=2)
        self._settings_text(frame, "Definitions JSON", "rules.process_definitions.definitions", 4, height=8)

    def _build_path_clarification_settings_section(self):
        frame = self._settings_section("Process Path Clarification")
        self._settings_check(frame, "rules.process_path_clarification.enabled", "Enabled", 0, 0)
        self._settings_combo(frame, "Severity", "rules.process_path_clarification.severity", 0, 1, SEVERITY_VALUES)
        self._settings_check(frame, "rules.process_path_clarification.auto_violation_pause", "Auto pause on violation", 1, 0, columnspan=2)
        self._settings_check(frame, "rules.process_path_clarification.allow_remote_kill", "Allow remote kill", 1, 2, columnspan=2)
    ...
    def _populate_settings_form_from_snapshot(self, *, force: bool = False):
        ...
        try:
            ...
            self._set_settings_text(
                "rules.process_definitions.definitions",
                json.dumps(process_definitions.get("definitions", []), indent=2, ensure_ascii=False),
            )
    ...
    def _collect_settings_payload(self) -> dict:
        ...
        if title_match_mode not in WINDOW_TITLE_MATCH_MODES:
            raise ValueError("Focused-window title match must be contains or exact.")
        ...
        exam_policy = {
            ...
            "rules": {
                ...
                "process_definitions": {
                    ...
                    "definitions": self._settings_json_list(
                        "rules.process_definitions.definitions",
                        "Process definitions",
                    ),
