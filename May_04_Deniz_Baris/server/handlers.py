...
async def login_handler(request: web.Request) -> web.Response:
    ...
    # Auth preflight: CATS verification + same-IP check + rate limit.
    # On CATS success, login_id is added to state.allowed_users so the
    # existing allowed_users check below passes transparently.
    from . import baris_features
    ok, error_response = await baris_features.baris_login_preflight(
        request, login_id, password, state
    )
    if not ok:
...
async def websocket_handler(request: web.Request) -> web.WebSocketResponse:
    ...
    login_id_for_uuid, _ = state.find_user_by_uuid(client_id)
    state.clients[client_id] = {
        "ws": ws,
        "short_id": short_id,
        "ip": ip,
        "login_id": login_id_for_uuid or "",
