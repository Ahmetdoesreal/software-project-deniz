...
class ServerManager(tk.Tk):
    ...
    def _build_layout(self):
        ...
        ttk.Label(
            info_frame,
            text=f"Available IPv4s: {', '.join(self.host_options)}",
            wraplength=860,
        ...
        # Authentication & Security configuration panel
        auth_frame = ttk.LabelFrame(self, text="Authentication & Security")
        auth_frame.grid(row=7, column=0, columnspan=2, sticky=tk.EW, padx=10, pady=(12, 6))
        auth_frame.columnconfigure(1, weight=1)

        # Row 0: CATS toggle
        self.v_baris_cats = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            auth_frame,
            text="Activate CATS Authorization (school-wide token-based auth instead of allowed_users.json)",
            variable=self.v_baris_cats,
        ).grid(row=0, column=0, columnspan=2, sticky=tk.W, padx=10, pady=(8, 4))

        # Row 1: Same-IP check
        self.v_baris_ip_check = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            auth_frame,
            text="Prevent multiple instances from the same IP (anti-cheat)",
            variable=self.v_baris_ip_check,
        ).grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=10, pady=4)

        # Row 2: Admin token
        ttk.Label(auth_frame, text="Admin Token:").grid(
            row=2, column=0, sticky=tk.W, padx=10, pady=4)
        self.v_baris_admin_token = tk.StringVar(value="admin-token-2026")
        ttk.Entry(
            auth_frame, textvariable=self.v_baris_admin_token, show="*"
        ).grid(row=2, column=1, sticky=tk.EW, padx=(0, 10), pady=4)

        # Row 3: Rate limit
        ttk.Label(auth_frame, text="Login Rate-Limit (sec):").grid(
            row=3, column=0, sticky=tk.W, padx=10, pady=(4, 8))
        self.v_baris_rate_limit = tk.StringVar(value="1.0")
        ttk.Entry(auth_frame, textvariable=self.v_baris_rate_limit, width=8).grid(
            row=3, column=1, sticky=tk.W, padx=(0, 10), pady=(4, 8))
    ...
    def start_server(self):
        ...
        env = {
            ...
            "BARIS_USE_CATS": "1" if self.v_baris_cats.get() else "0",
            "BARIS_IP_CHECK": "1" if self.v_baris_ip_check.get() else "0",
            "BARIS_ADMIN_TOKEN": self.v_baris_admin_token.get().strip() or "admin-token-2026",
            "BARIS_RATE_LIMIT_SEC": str(self.v_baris_rate_limit.get() or "1.0"),
