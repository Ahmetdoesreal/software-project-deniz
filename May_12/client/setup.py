"""
Simple setup entry point for the May_12 client bundle.
...
def validate_offline_source(source: Path) -> Path:
    ...
    ffmpeg = source / "ffmpeg" / "bin" / ("ffmpeg.exe" if os.name == "nt" else "ffmpeg")
    if ffmpeg.exists():
        print(f"  {OK} Bundled FFmpeg available: {ffmpeg}")
    else:
        print(f"  {INFO} FFmpeg not found in offline package; recorder will use PATH or EXAM_FFMPEG_PATH.")
    return wheelhouse
...
def install_dependencies(args: argparse.Namespace) -> None:
    ...
    if args.dry_run:
        print(f"  {OK} Client setup dry run completed; no dependencies were installed.")
    else:
        print(f"  {OK} Client dependencies are installed for the current user.")
        print(f"  {INFO} User site-packages: {site.getusersitepackages()}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Set up the May_12 client bundle.")
