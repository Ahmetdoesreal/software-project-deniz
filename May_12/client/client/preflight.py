...
def auth_status_disabled_modes(auth_status: dict | None) -> list[str]:
    if not isinstance(auth_status, dict):
        return []
    modes = []
    if str(auth_status.get("cats_bypass_until") or "").strip():
        modes.append("CATS")
    if str(auth_status.get("ad_bypass_until") or "").strip():
        modes.append("Windows auth")
    return modes


def auth_status_display_message(auth_status: dict | None) -> str:
    if not isinstance(auth_status, dict):
        return ""
    validation_status = str(auth_status.get("validation_status") or "").strip().lower()
    if validation_status == "denied":
        return "Credentials were denied by the admin."
    modes = auth_status_disabled_modes(auth_status)
    if not modes:
        return ""
    if len(modes) == 1:
        mode_text = modes[0]
    else:
        mode_text = " and ".join(modes)
    if bool(auth_status.get("admin_validation_required", False)):
        if validation_status == "approved":
            return f"Server temporarily disabled {mode_text}; admin approval is active."
        return f"Server temporarily disabled {mode_text}; waiting for admin approval."
    return f"Server temporarily disabled {mode_text}."
