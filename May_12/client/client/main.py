...
from .preflight import auth_status_display_message, fetch_auth_status
...
async def establish_session(base_url: str, args, recorder_manager: RecorderManager) -> str:
    ...
    while True:
        ...
        if isinstance(auth_status, dict):
            ...
            notice = auth_status_display_message(auth_status)
            if notice and notice != auth_status_notice:
                print(f"[AUTH] {notice}")
                auth_status_notice = notice
