...
class DashboardPopupMixin:
    ...
    def show_process_decision_window(self):
        ...
        process_name_var = tk.StringVar(value=str(row.get("process_name") or row.get("normalized_process_name") or ""))
        ...
        ttk.Label(controls, text="Executable / wildcard").grid(row=0, column=0, sticky=tk.W, padx=8, pady=6)
        ttk.Entry(controls, textvariable=process_name_var).grid(
            row=0,
            column=1,
            columnspan=3,
            sticky=tk.EW,
        ...
        ttk.Label(controls, text="Status").grid(row=1, column=0, sticky=tk.W, padx=8, pady=6)
        ttk.Combobox(
            controls,
            ...
            state="readonly",
            width=18,
        ).grid(row=1, column=1, sticky=tk.EW, padx=(0, 12), pady=6)

        ttk.Label(controls, text="Match Scope").grid(row=1, column=2, sticky=tk.W, padx=8, pady=6)
        ttk.Combobox(
            controls,
            ...
            state="readonly",
            width=18,
        ).grid(row=1, column=3, sticky=tk.EW, padx=(0, 12), pady=6)

        ttk.Checkbutton(controls, text="Ban", variable=action_vars["ban"]).grid(row=2, column=0, sticky=tk.W, padx=8, pady=6)
        ttk.Checkbutton(controls, text="Kick", variable=action_vars["kick"]).grid(row=2, column=1, sticky=tk.W, padx=8, pady=6)
        ttk.Checkbutton(controls, text="Pause Exam", variable=action_vars["pause_exam"]).grid(row=2, column=2, sticky=tk.W, padx=8, pady=6)
        ttk.Checkbutton(controls, text="Kill PID", variable=action_vars["kill_pid"]).grid(row=2, column=3, sticky=tk.W, padx=8, pady=6)
        ttk.Checkbutton(controls, text="Save decision to policy", variable=save_var).grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=8, pady=6)

        ttk.Button(
            controls,
            text="Google Search",
            command=lambda: webbrowser.open(process_row_google_search_url(row)),
        ).grid(row=3, column=2, sticky=tk.EW, padx=8, pady=6)
        ttk.Button(
            controls,
            text="Apply Policy",
            command=lambda: self._emit_process_decision(
                ...
                save_var.get(),
                process_name_var.get(),
            ),
        ).grid(row=3, column=3, sticky=tk.EW, padx=8, pady=6)
