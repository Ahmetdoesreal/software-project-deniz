...
class ServerGUI(PolicySettingsMixin, DashboardPopupMixin, QMainWindow):
    ...
    def _restore_scrollbars(self, widget, vertical_value: int, horizontal_value: int) -> None:
        ...
        QTimer.singleShot(0, lambda: restore(delayed=True))

    def _install_scroll_tracking(self, widget) -> None:
    ...
    def _build_rule_breakings_tab(self) -> None:
        ...
        self._install_scroll_tracking(self.incident_detail_table)
    ...
    def _update_incident_detail(self) -> None:
        if self._incident_tree_refreshing:
            return
        incidents = self._selected_incidents()
        vertical_value = self.incident_detail_table.verticalScrollBar().value()
        horizontal_value = self.incident_detail_table.horizontalScrollBar().value()
        rows: list[tuple[str, str]] = []
        if len(incidents) == 1:
