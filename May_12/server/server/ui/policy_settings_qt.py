...
class ProcessDecisionDialog(QDialog):
    ...
    def _build_layout(self):
        ...
        row0 = QHBoxLayout()
        row0.addWidget(QLabel("Executable / wildcard:"))
        self.process_name_edit = QLineEdit(str(self.row.get("process_name") or self.row.get("normalized_process_name") or ""))
        row0.addWidget(self.process_name_edit, stretch=1)
    ...
    def _on_apply(self):
        ...
        process_name = self.process_name_edit.text().strip() or str(
            self.row.get("process_name") or self.row.get("normalized_process_name") or ""
        ).strip()
...
class PolicySettingsDialog(QDialog):
    ...
    def _build_process_lists_tab(self):
        ...
        self._add_text(unexpected, "rules.unexpected_process.allowed_process_names", "Allowed process names or wildcards, one per line:")
    ...
    def _build_window_rules_tab(self):
        ...
        self._add_text(focused, "rules.focused_window.allowed_process_names", "Allowed process names or wildcards, one per line:", height=54)
        self._add_text(focused, "rules.focused_window.allowed_window_titles", "Allowed titlebar text, one per line:", height=54)
        self._add_text(focused, "rules.focused_window.blocked_process_names", "Blocked process names or wildcards, one per line:", height=54)
