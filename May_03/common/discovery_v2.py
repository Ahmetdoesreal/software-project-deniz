"""
discovery_v2.py -- isolated v2 baseline for the discovery rewrite.

This module is intentionally separate from the active runtime import so we can
reshape discovery without changing current behavior. The first v2 pass ignores
VPN / overlay-specific fallbacks and sticks to standard IPv4 interface metadata.
"""
...
class ServerAnnouncer:
    ...
    def _get_local_ip() -> str:
        """Best-effort local IPv4 detection using active IPv4 interfaces."""
    ...
    def _broadcast_targets(self) -> List[str]:
        ...
        directed_targets = [
            ...
            if entry.get("broadcast") and entry["broadcast"] != BROADCAST_ADDR
        ]
...
async def _wait_for_minimum_duration(started_at: float, duration: float):
    if duration <= 0:
        return
    remaining = started_at + duration - asyncio.get_running_loop().time()
    if remaining > 0:
...
async def check_duplicate_server(
    ...
    *,
    ...
    return its (host, port). Otherwise keep the startup guard interval before
    returning so the caller does not boot early on a fast no-result path.
    """
    ...
    if result is not None:
        return result
    await _wait_for_minimum_duration(started_at, timeout)
