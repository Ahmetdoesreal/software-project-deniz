...
def build_submission_bundle(
    ...
    *,
    ...
    _notify_step(step_callback, "3/6 Copying runtime logs and evidence files...")
    runtime_files = _snapshot_runtime_files_for_bundle(
        package_dir,
        runtime_files,
        step_callback=step_callback,
    )
    manifest = _build_bundle_manifest(staged_student_file, runtime_files)

    _notify_step(step_callback, "4/6 Creating archive from copied package folder...")
    with zipfile.ZipFile(bundle_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.write(staged_student_file, arcname=f"student_submission/{staged_student_file.name}")
        _write_manifest(archive, manifest)
        _add_runtime_files(archive, runtime_files)
    _notify_step(step_callback, "5/6 Archive ready with per-file checksums in manifest.")
...
def _snapshot_runtime_files_for_bundle(
    package_dir: Path,
    ...
    *,
    ...
    for runtime_file in runtime_files:
        ...
        staged_path = staging_dir / Path(runtime_file["arcname"])
        if runtime_file["arcname"] in staged_names:
            continue
        ...
        staged_names.add(runtime_file["arcname"])
        _notify_step(step_callback, f"   Copied: {runtime_file['arcname']}")
...
def _stage_student_submission(
    *,
    student_file: Path,
    package_dir: Path,
    step_callback: Callable[[str], None] | None = None,
) -> Path:
    _notify_step(step_callback, "2/6 Copying selected submission file...")
    destination = package_dir / "student_submission" / student_file.name
    destination.parent.mkdir(parents=True, exist_ok=True)
    if not _copy_runtime_file_with_retries(student_file, destination):
        raise ValueError(f"Failed to copy submission file into package folder: {student_file}")
    return destination
...
async def _post_file(
    *,
    ...
    file_size_mb = max(1, target_file.stat().st_size // (1024 * 1024))
    # Give large bundles enough time over slower LAN/Wi-Fi links.
    timeout_seconds = min(3600, 120 + file_size_mb * 2)
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        with target_file.open("rb") as file_handle:
            ...
            form.add_field("archive_sha256", checksum)
...
def _notify_step(step_callback: Callable[[str], None] | None, message: str):
    if step_callback:
        try:
